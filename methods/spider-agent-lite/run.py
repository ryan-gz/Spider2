...
def config() -> argparse.Namespace:
    ...
    parser.add_argument("--model", type=str, default="tablegpt2-7b")
    ...
    # example config
    parser.add_argument(
        "--test_path", "-t", type=str, default="./examples/spider2-lite.jsonl"
    )
    parser.add_argument(
        "--example_index",
        "-i",
    ...
    parser.add_argument(
        "--example_name", "-n", type=str, default="", help="name of the example to run"
    )
...
def test(args: argparse.Namespace, test_all_meta: dict = None) -> None:
    ...
    if args.example_name != "":
        task_configs = [
            task for task in task_configs if args.example_name in task["id"]
        ]
    else:
    ...
    for task_config in task_configs:
        ...
        task_type = None
        if task_config["instance_id"].startswith("bq") or task_config[
            "instance_id"
        ].startswith("ga"):
            task_type = "bq"
        ...
        env_config["init_args"]["name"] = (
            experiment_id + "-" + task_config["instance_id"]
        )
        ...
        task_config["config"] = [
            {
                ...
                "parameters": {
                    "dirs": [os.path.join(source_data_dir, task_config["instance_id"])]
                },
            }
        ]
