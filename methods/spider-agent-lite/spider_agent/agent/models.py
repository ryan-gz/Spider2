...
def call_llm(payload):
    ...
    elif model.startswith("azure") or model.startswith("o1") or model.startswith("gpt"):
        client = AzureOpenAI(
    ...
    #     return False, code_value
        ...
        for i in range(3):
            try:
                ...
                output_message = json.loads(response.json())["choices"][0]["message"][
                    "content"
                ]
                return True, output_message
            except Exception as e:
                ...
                if hasattr(e, "response") and e.response is not None:
                    ...
                    if code_value == "content_filter":
                        ...
                        if "content" in last_message and isinstance(
    ...
    elif model.startswith("tablegpt"):
        messages = payload["messages"]
        max_tokens = payload["max_tokens"]
        top_p = payload["top_p"]
        temperature = payload["temperature"]
        ...
        for i, message in enumerate(messages):
            ...
            tablegpt_message = {
                "role": message["role"],
                "content": (
                    message["content"][0]["text"][:2000]  # 截取2k，怕太长出错
                    if len(message["content"]) > 0
                    else ""
                ),
            }
            tablegpt_messages.append(tablegpt_message)

        for i in range(3):  # 重试机制
            try:
                logger.info("Generating content with model: %s", model)

                # 调用 LangChain OpenAI 模型
                response = llm.invoke(tablegpt_messages).content
                # print(response)
